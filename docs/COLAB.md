# Colab runbook — Music-CRS experiments

Generate fresh upload archives from the repository root:

| Zip | Purpose |
|-----|---------|
| **`artifacts/colab/music-crs-baselines.zip`** | Latest code bundle (`recall_at_k.py`, configs, and training scripts) |
| **`artifacts/colab/music-crs-evaluator.zip`** | Devset evaluator and catalog-diversity verifier |

Regenerate on macOS/Linux:
...
Run `./scripts/pack_colab_zips.sh`, then upload:

- `artifacts/colab/music-crs-baselines.zip`
- `artifacts/colab/music-crs-evaluator.zip`
...
**Or re-upload** `artifacts/colab/music-crs-baselines.zip` after running
`./scripts/pack_colab_zips.sh`, extract it, then check `VERSION.txt`.
